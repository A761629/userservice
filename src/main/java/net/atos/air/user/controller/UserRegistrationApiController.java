	...
	private ResponseEntity<Map<String, String>> userRegistration(@Valid @RequestBody UserDetailsdto users) {
		if (!userService.findByUserName(users.getUserName())) {
			...
			return new ResponseEntity<Map<String, String>>(
					Collections.singletonMap("Success", env.getProperty("successMessage")), HttpStatus.CREATED);
		} else
			return new ResponseEntity<Map<String, String>>(
					Collections.singletonMap("Error", env.getProperty("userexist")), HttpStatus.ALREADY_REPORTED);
	}
